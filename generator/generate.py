    ...
    def build_execution_plan(self) -> list:
        ...
        for comp_id in exec_order:
            ...
            if 'tLoop' in comp_type:
                ...
                step['directory'] = fl.get('directory', '')
                step['filemask'] = fl.get('filemask', '')
                step['order_by'] = fl.get('order_by_filename', '')
            elif 'tFileCopy' in comp_type:
                fc = comp.get('file_copy', {})
                step['action'] = 'FILE_COPY'
                step['source'] = fc.get('filename', '') or fc.get('source_directory', '')
                step['destination'] = fc.get('destination', '')
                step['rename'] = fc.get('rename', '')
                step['remove_source'] = fc.get('remove_file', 'false')
            elif 'tFileDelete' in comp_type:
                fd = comp.get('file_delete', {})
                step['action'] = 'FILE_DELETE'
                step['path'] = fd.get('filename', '') or fd.get('path', '')
                step['failon'] = fd.get('failon', 'true')
            elif 'tFileArchive' in comp_type:
                fa = comp.get('file_archive', {})
                step['action'] = 'FILE_ARCHIVE'
                step['source'] = fa.get('source', '')
                step['target'] = fa.get('target', '')
                step['format'] = fa.get('archive_format', 'zip')
                step['encrypt'] = fa.get('encrypt_files', 'false')
                step['encrypt_method'] = fa.get('encrypt_method', '')
                step['aes_strength'] = fa.get('aes_key_strength', '')
                step['password'] = fa.get('password', '')
                ...
                step['host'] = dc.get('host', '')
                step['database'] = dc.get('database', '')
            else:
