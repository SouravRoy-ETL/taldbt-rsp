...
def _str_val(val) -> str:
    ...
    Handles: str, dict, None, int, float, bool."""
    if val is None:
        return ''
...
class ConnectionGraph:
    ...
    def build_execution_plan(self) -> list:
        ...
        for comp_id in exec_order:
            ...
            # Normalize component type for matching (handles case variations)
            ct_lower = comp_type.lower()

            # Enrich with component-specific metadata
            if 'tloop' in ct_lower:
                loop = comp.get('loop', {})
                step['action'] = 'LOOP'
            ...
            elif 'tparallelize' in ct_lower:
                par = comp.get('parallelize', {})
                step['action'] = 'PARALLELIZE'
                step['wait_for'] = par.get('wait_for', '')
                step['targets'] = [e['target'] for e in outgoing]
            elif 'tfilelist' in ct_lower:
                fl = comp.get('file_list', {})
                step['action'] = 'FILE_LIST'
                step['directory'] = _str_val(fl.get('directory', ''))
                step['filemask'] = _str_val(fl.get('filemask', ''))
                step['order_by'] = fl.get('order_by_filename', '')
            elif 'tfilecopy' in ct_lower:
                fc = comp.get('file_copy', {})
                step['action'] = 'FILE_COPY'
                step['source'] = _str_val(fc.get('filename', '')) or _str_val(fc.get('source_directory', ''))
                step['destination'] = _str_val(fc.get('destination', ''))
                step['rename'] = _str_val(fc.get('rename', ''))
                step['remove_source'] = _str_val(fc.get('remove_file', 'false'))
            elif 'tfiledelete' in ct_lower:
                fd = comp.get('file_delete', {})
                step['action'] = 'FILE_DELETE'
                step['path'] = _str_val(fd.get('filename', '')) or _str_val(fd.get('path', ''))
                step['failon'] = _str_val(fd.get('failon', 'true'))
            elif 'tfilearchive' in ct_lower:
                fa = comp.get('file_archive', {})
                step['action'] = 'FILE_ARCHIVE'
                step['source'] = _str_val(fa.get('source', ''))
                step['target'] = _str_val(fa.get('target', ''))
                step['format'] = _str_val(fa.get('archive_format', 'zip'))
                step['encrypt'] = _str_val(fa.get('encrypt_files', 'false'))
                step['encrypt_method'] = _str_val(fa.get('encrypt_method', ''))
                step['aes_strength'] = _str_val(fa.get('aes_key_strength', ''))
                step['password'] = _str_val(fa.get('password', ''))
            elif 'trunjob' in ct_lower:
            ...
            elif 'tchronometerstop' in ct_lower:
                cs = comp.get('chronometer', {})
                step['action'] = 'CHRONOMETER'
                step['since'] = cs.get('since', '')
            elif 'tprejob' in ct_lower:
                step['action'] = 'PREJOB'
            elif 'tpostjob' in ct_lower:
                step['action'] = 'POSTJOB'
            elif 'tdbconnection' in ct_lower or 'tsnowflakeconnection' in ct_lower:
                dc = comp.get('db_connection', {})
                step['action'] = 'DB_CONNECTION'
                step['host'] = _str_val(dc.get('host', ''))
                step['database'] = _str_val(dc.get('database', ''))
            else:
    ...
    def _collect_source_tables(self):
        ...
        for job_name, job_data in self.parsed_items.items():
            ...
            for comp_id, comp in components.items():
                ...
                if dc:
                    conn_map[comp_id] = {
                        'database': _str_val(dc.get('database', '')),
                        'schema': _str_val(dc.get('schema', '')),
                    }
            ...
            for comp_id, comp in components.items():
                for key in ('db_input', 'db_output'):
                    ...
                    conn_ref = _str_val(sub.get('connection_ref', ''))
...
'''
    ...
    def _gen_job_models(self, job_name: str, job_data: dict):
        ...
        for comp_id in exec_order:
            ...
            # Route EVERY component type (case-insensitive to handle Talend naming variations)
            if 'tmap' in ct_lower and 'row' not in ct_lower:
                self._gen_tmap_model(mn, comp, graph, comp_id, subdir)
            elif 'tjavarow' in ct_lower:
                self._gen_javarow_model(mn, comp, graph, comp_id, subdir)
            elif 'tjava' in ct_lower and 'row' not in ct_lower:
                self._gen_tjava_model(mn, comp, comp_id, subdir)
            elif 'dbrow' in ct_lower or 'snowflakerow' in ct_lower:
                self._gen_dbrow_model(mn, comp, comp_id, subdir)
            elif 'dboutput' in ct_lower or 'snowflakeoutput' in ct_lower:
                self._gen_dboutput_model(mn, comp, graph, comp_id, subdir)
            elif 'dbinput' in ct_lower or 'snowflakeinput' in ct_lower:
                self._gen_dbinput_source(mn, comp, comp_id, subdir)
            elif 'tuniqrow' in ct_lower:
                self._gen_uniqrow_model(mn, comp, graph, comp_id, subdir)
            elif 'fileinputdelimited' in ct_lower or 'fileinputfullrow' in ct_lower:
                self._gen_file_source(mn, comp, comp_id, subdir)
            elif 'fileoutputdelimited' in ct_lower:
                self._gen_file_output_model(mn, comp, graph, comp_id, subdir)
            elif 'extractdelimitedfields' in ct_lower:
                self._gen_extract_fields_model(mn, comp, graph, comp_id, subdir)
            elif 'fixedflowinput' in ct_lower:
                self._gen_fixed_flow_model(mn, comp, comp_id, subdir)
            elif 'flowtoiterate' in ct_lower or 'iteratetoflow' in ct_lower:
                self._gen_flow_iterate_model(mn, comp, graph, comp_id, subdir)
            elif 'bufferoutput' in ct_lower or 'logrow' in ct_lower:
                # tBufferOutput + tLogRow = passthrough/debug components
                self._gen_buffer_model(mn, comp, graph, comp_id, subdir)
            elif 'fileexist' in ct_lower:
                # tFileExist — orchestration check, register in plan
                for step in plan:
                    if step['comp_id'] == comp_id:
                        step['handled'] = 'execution_plan'
                        step['action'] = 'FILE_EXIST'
                self._register_model(comp_id, mn)
            elif any(kw in ct_lower for kw in [
                'filelist', 'filecopy', 'filedelete', 'filearchive',
                'parallelize', 'tloop', 'runjob', 'logcatcher',
                'sendmail', 'chronometerstop', 'prejob', 'postjob',
                'dbconnection', 'snowflakeconnection',
            ]):
                # Orchestration components → handled by execution_plan
                for step in plan:
                    if step['comp_id'] == comp_id:
                        step['handled'] = 'execution_plan'
                self._register_model(comp_id, mn)
            else:
                # Unknown component — register it anyway so refs don't break
                print(f"    [!] Unknown: {ct} ({comp_id}) — flagged in plan")
                for step in plan:
                    if step['comp_id'] == comp_id:
                        step['action'] = 'UNKNOWN'
                        step['warning'] = f'Unhandled: {ct}'
                self._register_model(comp_id, mn)
    ...
    def _gen_javarow_model(self, model_name, comp, graph, comp_id, subdir):
        ...
        code = _str_val(comp.get('java_code', ''))
        if not code:
            return
        ctx = _str_val(comp.get('unique_name', ''))
    ...
    def _gen_dbrow_model(self, model_name, comp, comp_id, subdir):
        ...
        sql = _str_val(comp.get('sql_query', ''))
        if not sql:
            return
    ...
    def _gen_dboutput_model(self, model_name, comp, graph, comp_id, subdir):
        ...
        table = _str_val(db_out.get('table', ''))
        action = (_str_val(db_out.get('output_action', '')) or _str_val(db_out.get('table_action', '')) or 'INSERT').upper()
    ...
    def _gen_dbinput_source(self, model_name, comp, comp_id, subdir):
        ...
        if not db_in:
            return
        query = _str_val(db_in.get('query', ''))
        table = _str_val(db_in.get('table', ''))
        conn_ref = _str_val(db_in.get('connection_ref', ''))
    ...
    def _gen_file_source(self, model_name, comp, comp_id, subdir):
        ...
        lines.append(f"-- Filename: {_str_val(fi.get('filename', ''))}")
        lines.append(f"-- Separator: {_str_val(fi.get('fieldseparator', ''))}")
        lines.append(f"-- Encoding: {_str_val(fi.get('encoding', ''))}")
        sep = _str_val(fi.get('fieldseparator', ',')).strip('"').replace("';'", ';')
        hdr = 'true' if _str_val(fi.get('header', '0')) != '0' else 'false'
    ...
    def _gen_file_output_model(self, model_name, comp, graph, comp_id, subdir):
        ...
        sources = graph.get_data_sources(comp_id)
        src_ref = self._resolve_ref(sources[0]['source']) if sources else None
        filename = _str_val(fo.get('filename', ''))
        sep = _str_val(fo.get('fieldseparator', ',')).strip('"')
    ...
    def _gen_extract_fields_model(self, model_name, comp, graph, comp_id, subdir):
        ...
        sources = graph.get_data_sources(comp_id)
        src_ref = self._resolve_ref(sources[0]['source']) if sources else None
        field_sep = _str_val(ef.get('fieldseparator', ',')).strip('"')
    ...
    def _gen_fixed_flow_model(self, model_name, comp, comp_id, subdir):
        ...
        if not ff:
            return
        nb_rows = _str_val(ff.get('nb_rows', '1'))
